...
```{r}
Xaxis <- BusinessDF[, "MedianHome"]
Yaxis <- BusinessDF[, "BusinessFormed"]
...
```{r}
testBDF <- BusinessDF[,c(2:4)]
head(testBDF)
```

```{r}
clusterResult <- kmeans(testBDF, 3)
clusterResult$size
```
###################################### My new file
...
```{r}
table(result$cluster, growthGroup)
```
...
dt = sort(sample(nrow(finalDF2), nrow(finalDF2)*.7))
train<-finalDF2[dt,]
test<-finalDF2[-dt,]
#view(train)
train
...
```{r}
normalizedNums <- function(x){
  (x-min(x))/(max(x)-min(x))
}
normTrain <- as.data.frame(lapply(train[1:4], normalizedNums))
normTrain
```
...
normTest <- as.data.frame(lapply(test[1:4], normalizedNums))
normTest
```

```{r}
predicted1 <- predict(model1, normTest)
predicted1
```
